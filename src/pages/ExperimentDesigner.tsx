...
import { useAlgorithms, useEnvironments, useInspectDesign, useNetworks, useWrappers } from '@/api/hooks'
...
import type { AlgorithmSpec, EnvKind, EnvSpec, WrapperNode as WrapperNodeSpec } from '@/api/types'
...
export function ExperimentDesigner() {
  ...
  }, [compatibleAlgorithms.map((a) => a.id).join(','), algorithmId, selectedEnv])
  ...
  const handleRun = useCallback(async () => {
    ...
    try {
      const config = {
        ...
        algorithm: { id: algorithmId, hyperparams, network_spec_id: networkSpecId },
        training: {
  ...
  }, [canRun, kind, name, environmentId, wrappers, algorithmId, hyperparams, networkSpecId, totalTimesteps, numIterations, seed, useGpu, navigate])
  ...
  const rawNodes: Node[] = useMemo(() => {
    ...
    // A "+" sits in every gap of the env→wrapper→...→algorithm chain — before
    // the first wrapper, between each pair, and right before the algorithm —
    // so a wrapper can be inserted anywhere, not just appended at the end.
    if (kind === 'gym') {
      const chainX = [COL_X[0], ...wrappers.map((_, i) => wrapperColumnBase + i * WRAPPER_GAP)]
      for (let k = 0; k <= wrappers.length; k++) {
        const leftX = chainX[k]
        const rightX = k < wrappers.length ? chainX[k + 1] : algoX
        list.push({
          id: `add-wrapper-${k}`,
          type: 'add',
          position: { x: (leftX + rightX) / 2, y: ROW_Y + 140 },
    ...
    hyperparams, networks, networkSpecId, kind, name, totalTimesteps, numIterations, seed, useGpu, starting, canRun, addWrapper,
  ])
